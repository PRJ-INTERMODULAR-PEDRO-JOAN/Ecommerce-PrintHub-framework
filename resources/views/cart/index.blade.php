@extends('layouts.legacy')

@section('title', 'Mi Carrito')

@section('content')
<div class="contenido-principal container py-5">
    <h2 class="mb-5 fw-bold text-center">🛒 Tu Carrito de Compra</h2>

    @if(count($cart) > 0)
        <div class="row g-4">
            <div class="col-lg-8">
                <div class="card border-0 shadow-sm rounded-4 overflow-hidden">
                    <div class="card-header bg-white py-3">
                        <h5 class="m-0 fw-bold text-muted">Productos ({{ count($cart) }})</h5>
                    </div>
                    <div class="card-body p-0">
                        @foreach($cart as $id => $details)
                            <div class="d-flex align-items-center p-3 border-bottom position-relative hover-bg-light">
                                <div class="flex-shrink-0">
                                    <img src="{{ asset('img/' . $details['image']) }}" 
                                         alt="{{ $details['name'] }}" 
                                         class="rounded-3 border" 
                                         style="width: 100px; height: 100px; object-fit: contain; background: #fff;">
                                </div>
                                
                                <div class="flex-grow-1 ms-3">
                                    <h5 class="fw-bold mb-1">{{ $details['name'] }}</h5>
                                    <p class="text-muted small mb-2">Precio unidad: {{ $details['price'] }} €</p>
                                    
                                    <div class="d-flex align-items-center">
                                        <span class="badge bg-light text-dark border px-3 py-2 rounded-pill">
                                            Cantidad: {{ $details['quantity'] }}
                                        </span>
                                    </div>
                                </div>

                                <div class="text-end ms-3">
                                    <h4 class="text-primary fw-bold mb-2">
                                        {{ $details['price'] * $details['quantity'] }} €
                                    </h4>
                                    <a href="{{ route('cart.remove', $id) }}" 
                                       class="text-danger text-decoration-none small fw-bold hover-underline">
                                        Eliminar 🗑️
                                    </a>
                                </div>
                            </div>
                        @endforeach
                    </div>
                </div>
            </div>
            
            <div class="col-lg-4">
                <div class="card border-0 shadow rounded-4 sticky-top" style="top: 100px; z-index: 1;">
                    <div class="card-body p-4">
                        <h4 class="fw-bold mb-4">Resumen</h4>
                        
                        <div class="d-flex justify-content-between mb-2">
                            <span class="text-muted">Subtotal</span>
                            <span class="fw-bold">{{ $total }} €</span>
                        </div>
                        <div class="d-flex justify-content-between mb-4">
                            <span class="text-muted">Envío</span>
                            <span class="text-success fw-bold">Gratis</span>
                        </div>
                        
                        <hr class="my-4">
                        
                        <div class="d-flex justify-content-between align-items-center mb-4">
                            <span class="fs-5 fw-bold">Total a Pagar</span>
                            <span class="fs-2 fw-bold text-primary">{{ $total }} €</span>
                        </div>
                        
                        <div class="d-grid gap-2">
                            <a href="{{ route('cart.checkout') }}" class="btn btn-primary btn-lg rounded-pill fw-bold shadow-sm">
                                Tramitar Pedido →
                            </a>
                            <a href="{{ route('products.list') }}" class="btn btn-outline-secondary rounded-pill">
                                Seguir Comprando
                            </a>
                        </div>
                        
                        <div class="mt-4 text-center text-muted small">
                            🔒 Pago 100% Seguro garantizado
                        </div>
                    </div>
                </div>
            </div>
        </div>
    @else
        <div class="text-center py-5">
            <div class="mb-4 display-1 text-muted opacity-25">🛒</div>
            <h3 class="fw-bold">Tu carrito está vacío</h3>
            <p class="text-muted mb-4">¡Añade productos increíbles a tu colección!</p>
            <a href="{{ route('products.list') }}" class="btn btn-primary btn-lg rounded-pill px-5">Ir al Catálogo</a>
        </div>
    @endif
</div>

<style>
    .hover-bg-light:hover { background-color: #f8f9fa; transition: 0.3s; }
    .hover-underline:hover { text-decoration: underline !important; }
</style>
@endsection